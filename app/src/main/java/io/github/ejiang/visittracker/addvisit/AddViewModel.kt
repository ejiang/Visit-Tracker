...
import io.github.ejiang.visittracker.util.Utils.Companion.calcTimeFrom
import io.github.ejiang.visittracker.util.Utils.Companion.currencyFormat
...
class AddViewModel: ViewModel() {
    ...
    fun start(vdb: RestTrackDB, addEdit: AddEdit) {
        this.vdb = vdb

        if (addEdit == AddEdit.ADD) // normal; if it's edit, it's already loaded
    ...
    fun saveNote(addEdit: AddEdit) {
        when (addEdit) {
            ...
            AddEdit.EDIT -> {
                ...
                vdb.rtDao().updateVisit(vid, note=note, timestamp=date, spending=spending2)
            }
        }
    }
